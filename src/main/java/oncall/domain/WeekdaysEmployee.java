package oncall.domain;

import oncall.util.ErrorMessage;

import java.util.ArrayList;
import java.util.List;

public class WeekdaysEmployee {
    private final List<Employee> employees;

    private WeekdaysEmployee(List<Employee> employees) {
        this.employees = employees;
    }

    public static WeekdaysEmployee of(List<String> employeesStr) {
        List<Employee> employees = new ArrayList<>();
        for (String employeeStr : employeesStr) {
            Employee employee = Employee.of(employeeStr);
            employees.add(employee);
        }
        validate(employees);
        return new WeekdaysEmployee(employees);
    }

    private static void validate(List<Employee> employees) {
        validateNumberOfEmployees(employees);
        validateDuplicate(employees);
    }

    private static void validateNumberOfEmployees(List<Employee> employees) {
        if (employees.size() < 5 || employees.size() > 35) {
            throw new IllegalArgumentException(ErrorMessage.INVALID_INPUT_EXCEPTION.getMessage());
        }
    }

    private static void validateDuplicate(List<Employee> employees) {
        if (employees.stream().distinct().count() != employees.size()) { // 중복 제거 후 개수 카운트
            throw new IllegalArgumentException(ErrorMessage.INVALID_INPUT_EXCEPTION.getMessage());
        }
    }

    @Override
    public String toString() {
        return "WeekdaysEmployee{" +
                "employees=" + employees +
                '}';
    }
}
